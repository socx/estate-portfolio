...
const Datepicker = ({
  ...
  const options = {
    ...
    onReady: (selectedDates, dateStr, instance) => {
      instance.element.value = mode === 'single'
        ? dateStr.replace('to', '-')[0]
        : dateStr.replace('to', '-');
      ...
      if (handleDatePickerReady) handleDatePickerReady(selectedDates);
    },
    onChange: (selectedDates, dateStr, instance) => {
      instance.element.value = dateStr.replace('to', '-');
      if (handleDatePickerChange) handleDatePickerChange(selectedDates[0], selectedDates[1]);
    },
  };
